                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card-body p-0"style="overflow: auto">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @if (count($guardianStudents) > 0)

                                        @foreach ($guardianStudents as $guardianStudent)
                                            <tr>
                                                <td>{{ $guardianStudent->id }}</td>
                                                <td>{{ $guardianStudent->name }}</td>
                                                <td>{{ $guardianStudent->last_name }}</td>
                                                <td>{{ $guardianStudent->email }}</td>

                                                <td>
                                                    <form
                                                        action={{ url('admin/guardians/students/remove/' . $guardianId . '/' . $guardianStudent->id) }}
                                                        method="get">
                                    ...
                                    @foreach ($students as $student)
                                        <tr>
                                            ...
                                            <td>
                                                <form
                                                    action={{ url('admin/guardians/students/add/' . $guardianId . '/' . $student->id) }}
                                                    method="get">
                                                    {{ @csrf_field() }}
